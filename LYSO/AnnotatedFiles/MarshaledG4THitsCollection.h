...
void marshal1() {
	...
	int copy_off = 0;
	int elementNum;
	 elementNum = ((G4THitsCollection<LYSOHit>*)param)->entries(); 
	memcpy( msh_cursor+copy_off, &elementNum,sizeof(int));
	copy_off += sizeof(int);
	for(int index=0;index<elementNum;index++){
			LYSOHit*   anElement;
			 anElement = (*((G4THitsCollection<LYSOHit>*)param))[index]; 
			MarshaledLYSOHit   marEle(anElement);
...
void unmarshal1() {
    ...
    //Now copy the setspec here
    {
		...
		copy_off += sizeof(int);
		for(int index=0;index<elementNum;index++){
			MarshaledLYSOHit   marEle(msh_cursor+copy_off);
			LYSOHit*   anElement = (LYSOHit*  )marEle.unmarshal();
			copy_off += marEle.getBufferSize();
			 ((G4THitsCollection<LYSOHit>*)param)->insert((LYSOHit*)anElement); 
		}
